...
def index():
    ...
    <head>
        ...
        <style>
            * { margin:0; padding:0; box-sizing:border-box; }
            ...
            .subtitle { color: #a0aec0; font-size: 14px; margin-bottom: 25px; }
            ...
            .checkbox-group input { width: auto; accent-color: #f7971e; }
            .btn {
                padding: 12px 28px;
            ...
            .proxy-options input { flex: 1; min-width: 100px; }
            ...
            .copy-btn:hover { background: #3572b0; }
        </style>
    </head>
    <body>
    <div class="container">
        <h1>🚀 Buff Pro</h1>
        <p class="subtitle">Tăng view / like Facebook & TikTok với proxy</p>
        ...
        <!-- Khu vực copy proxy -->
        <div id="copyArea" class="hidden" style="margin-bottom: 16px;">
            <label>📋 Danh sách proxy đang dùng</label>
            <div class="copy-area">
                <input type="text" id="proxyText" readonly value="">
                <button class="copy-btn" onclick="copyProxy()">📋 Sao chép</button>
            </div>
        </div>
    ...
    <script>
        ...
        function copyProxy() {
            const input = document.getElementById('proxyText');
            input.select();
            document.execCommand('copy');
            alert('✅ Đã sao chép proxy vào clipboard!');
        }
...
def buff():
    ...
    if use_auto_proxy:
        ...
        if not proxy_list:
            return jsonify({'error': 'Không lấy được proxy, hãy nhập thủ công'}), 400
    else:
    ...
    task_data = {'url': url, 'count': count, 'proxies': proxy_list}
